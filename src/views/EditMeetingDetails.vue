<template>
    <div class="meeting-view">
      ...
      <div class="meeting-details-container">
        ...
        <button @click="RedirectToOfficialLook(meeting.id)" class="look">Guests View</button>
        <button @click="RedirectToEditMeetingInfo(meeting.id)" class="edit-meeting">Edit Meeting Info</button>
        </div>
        <hr>
      </div>
      
      <div class="points-container">
      <h2 style="padding-left: 10px;">Točke dnevnog reda:        <button class="add-points-button"  @click="redirectToMeetingDetails(meeting.id)">Add Points</button></h2>
      <div v-for="(point, index) in meeting.points" :key="point.id" class="point">
        <hr>
        <div class="point-details">
          <h3 style="font-size: 24px;">{{ index + 1 }}. {{ point.title }}    <button @click="editPoint(point.id)" class="edit-point">Edit Point</button>    </h3>
          <p style="font-size: 20px;">{{ point.details }}</p>
          

          <!-- Display Materials for Points -->
          <div class="point-materials">
            <h4>Materijali:    <button @click="editMaterials(point.id)" class="edit-materials">Edit Materials</button></h4>
            <ul>
          ...
          <div class="subpoints">
            <div v-if="point.subpoints.length > 0">
              <div v-for="(subpoint, subIndex) in point.subpoints" :key="subpoint.id" class="subpoint">
                <h3 style="font-size: 16px;">{{ index + 1 }}.{{ subIndex + 1 }} {{ subpoint.title }}   </h3>
                <p>{{ subpoint.details }}</p>

                <!-- Display Materials for Subpoints -->
                <div class="subpoint-materials">
                  <h4>Materijali:  <button @click="editMaterials(subpoint.id)" class="edit-materials">Edit Materials</button></h4>
                  <ul>
            ...
            <div v-else>
              <p>No subpoints for this point.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      axios
        ...
        .then((response) => {
          ...
          axios
            ...
            .catch((pointsError) => {
              console.error("Error fetching meeting top level points:", pointsError);
            });
        })
        .catch((error) => {
